...
//Returns an array of ars for the setLength funcion of an ArrowHelper
...
class Body {
  constructor(mass, rx, ry, rz, vx, vy, vz, showArrow=true) {
    ...
    //Mesh
    //Make first arg proportional to mass
  ...
  //Update velocity components of the body
  updateVelocity(v) {
    this.velocity = v;
...
class Trail {
  constructor(rX, rY, rZ, colour){
    //Set the size of the trail
      ...
      //Create the line geometry used for storing vertices
...
//Describes derivatives for a body
...
function solve(y0) {
  ...
  //Enable dense output to extract new positions and velocities for each time step
...
//Returns an array that encoes the bodies movement
